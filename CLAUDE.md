...
Homebrew tap `edbfi/taps`: macOS casks that re-host upstream DMGs on this repo's releases, plus Linux-only source formulae. No application code lives here; the moving parts are the casks/formulae and a Bash update pipeline (`scripts/`, `pipelines/<token>/`), run manually; no workflows run it.
...
Native cask checks (macOS only) need this checkout tapped as `edbfi/taps`; don't clobber an existing local `edbfi/homebrew-taps` tap, and remove only your own symlink afterwards:
...
- `version` and `sha256` in `Casks/**/<token>.rb` are machine-owned. Move them only through the update pipeline, run manually in this order: `scripts/resolve.sh`, `check-update.sh`, `fetch-asset.sh`, `inspect-macos.py`, `release-notes.sh`, `publish-release.sh`, `write-cask.sh`. Propose the result as a PR.
...
- A same-version upstream re-release is never picked up: `check-update.sh` reports `needed=false` once the asset exists, and `publish-release.sh` keeps a same-named asset (it fails if the hosted bytes differ). To recover, delete that asset from the `<token>-latest` release and rerun the pipeline for the cask.
...
README's "three files" list is incomplete. A new token touches:
...
5. `scripts/tests/test_pipeline.py`: the cask count in `test_discovery_separates_kinds`, plus a `ResolverTests` case for the new guards.
6. `README.md` tables (Support, Apps, License).

After merge, the first pipeline run creates the rolling release and rewrites version/sha256, because the missing release counts as "update needed".

## Linux formulae

- `Formula/*.rb` are Linux-only (`depends_on :linux`) and pinned to commit-SHA archive URLs with an explicit `version`. To bump a source by hand, run from the repo root:
  ```bash
...
- Adding a formula means editing `Formula/<token>.rb`, adding `formula` to `PACKAGE_KINDS` and a `FORMULA_TAG_RE` in `config.env` (or a `FORMULA_SOURCE` branch in `resolve-formula.sh`). Build in dependency order (`pipewire-gstreamer` before `fcast-sender`). Also update `test_discovery_separates_kinds`. GUI apps additionally need the default list in `scripts/test-linux-gui.sh` and the `choices`/expected titles in `scripts/gui-smoke.py`.
- Build, test and bottle each formula immediately, one at a time. Never interleave other `brew install`s, because Homebrew attributes prefix changes to the bottle being built.
...
- Casks are updated one at a time by design; don't run the pipeline for several casks in parallel.
- `scripts/vendor/gh-workflow-immortality.sh` is vendored MIT code (v1.1.1), used unchanged. Re-vendor from upstream instead of patching it.
